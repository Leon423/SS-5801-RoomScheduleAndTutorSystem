...
function displayRooms(dateInput) {
    ...
    // API call
    xhttp.open("POST", "https://v7bt1yty0b.execute-api.us-east-2.amazonaws.com/prod/viewrooms", false);
...
async function submitRequest(button_id) {
    ...
    if(lastTimeBlock != -1) {
        ...
        if(timeBlockID.length > 0) {
            ...
            xhttp.open("POST", "https://v7bt1yty0b.execute-api.us-east-2.amazonaws.com/prod/submitrequest", "false");
            xhttp.send(JSON.stringify(dataUpload));

            
            //setTimeout(dateChanged, 1000);
        }
        else {
